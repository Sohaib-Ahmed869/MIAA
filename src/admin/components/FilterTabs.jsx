...
export default function FilterTabs({ options, value, onChange, className = "" }) {
  return (
    <div
      className={`inline-flex self-start items-center gap-0.5 flex-wrap bg-white border border-primary/10 rounded-full p-1 shadow-sm shadow-primary/5 ${className}`}
    >
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-4 py-1.5 text-[0.625rem] tracking-[0.18em] uppercase rounded-full transition-all duration-200 ${
              isActive
